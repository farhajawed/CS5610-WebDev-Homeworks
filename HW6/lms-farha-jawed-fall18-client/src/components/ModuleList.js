...
class ModuleList extends React.Component{
    ...
    componentDidUpdate(prevPros) {
        if(prevPros.courseId !== this.props.courseId) {
            ModuleService.findModule(this.props.courseId)
    ...
    formChanged = (event) => {
        this.setState({
            newModuleTitle: event.target.value||'New Module'
        })
    }
    ...
    addNewModule = () => {
        ...
        ModuleService.createModule(this.props.courseId,newModule).then(
    ...
    deleteModule = (moduleId) => {
        ModuleService.deleteModule(moduleId)
          .then(() => {
              ModuleService.findModule(this.props.courseId).then(modules=>this.setState({
                               modules:modules
                       })
             )})
     }
